          ...
          {hours.content
            ...
            .map((currentHour) => (
              <DayDisplay
                ...
                disabled={!selected}
                selected={selected && parseInt(hour) === currentHour}
                onHandleClick={() =>
                  handleClick(selected.clone().hour(currentHour), 'hour')
                }
              />
            ))}
        </ContainerBlock>
        <MinuteSelector disabled={!hour}>
          {minutes.map((currentMinute) => (
            <DayDisplay
              ...
              selected={selected && parseInt(minute) === currentMinute}
              onHandleClick={() =>
                handleClick(selected.clone().minute(currentMinute), 'minute')
              }
            />
          ))}
        </MinuteSelector>
