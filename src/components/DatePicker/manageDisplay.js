...
const manageDayDisplay = (
  ...
  const selectedFunction = (currentDay) =>
    selectedDate && currentDay.isSame(selectedDate, 'day')
  ...
  const disabledFunction = (currentDay, periodMoment) =>
    ...
    (props.disabledDates &&
      props.disabledDates.includes(currentDay.format('DD/MM/YYYY')))
...
const manageMonthDisplay = (periodMoments, selectedDate, updateFunction) => {
  ...
  const handleClickFunction = (currentDay) =>
    updateFunction(currentDay, MONTH_SELECTED)
  const selectedFunction = (currentDay) =>
    selectedDate && currentDay.isSame(selectedDate, 'month')
...
const manageYearDisplay = (periodMoments, selectedDate, updateFunction) => {
  ...
  const handleClickFunction = (currentDay) =>
    updateFunction(currentDay, YEAR_SELECTED)

  const selectedFunction = (currentDay) =>
    selectedDate && currentDay.isSame(selectedDate, 'year')
  const disabledFunction = () => 1 === 2 // (!currentDay.isSame(periodMoment, 'month') || !currentDay.isSameOrAfter(Moment(), 'day'))
